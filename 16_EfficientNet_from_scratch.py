...
class SqueezeExcitation(nn.Module):
    def __init__(self, in_channels, squeeze_dim):
        super(SqueezeExcitation, self).__init__()
        self.se = nn.Sequential(nn.AdaptiveAvgPool2d(1),  # C x H x W -> C x 1 x 1
                                nn.Conv2d(in_channels, squeeze_dim, 1),
                                nn.SiLU(),
                                nn.Conv2d(squeeze_dim, in_channels, 1),
                                nn.Sigmoid())
...
class InvertedResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels,
                 ...
                 expand_ratio, squeeze_ratio=4, survival_prob=0.8):
        super(InvertedResidualBlock, self).__init__()

        self.survival_prob = survival_prob
        hidden_dim = in_channels * expand_ratio
        squeeze_dim = int(in_channels / squeeze_ratio)
        ...
        self.conv = nn.Sequential(CNNBlock(hidden_dim, hidden_dim, kernel_size, stride, padding, groups=hidden_dim),
                                  SqueezeExcitation(hidden_dim, squeeze_dim),
    ...
    def stochastic_depth(self, x):
        ...
        binary_mask = torch.rand(x.shape[0], 1, 1, 1, device=x.device) < self.survival_prob
        return torch.div(x, self.survival_prob) * binary_mask
    ...
    def create_features(self, width_factor, depth_factor, last_channels):
        ...
        for expand_ratio, channels, repeats, stride, kernel_size in base_model:
            ...
            for layer in range(layers_repeats):
                features.append(InvertedResidualBlock(in_channels, out_channels, kernel_size=kernel_size,
                                                      stride=stride if layer == 0 else 1, padding=kernel_size // 2,
